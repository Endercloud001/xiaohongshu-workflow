...
凭据占位符必须完整匹配整个值，不能以前缀或子串形式夹带真实内容。对普通解构、重命名解构和成员别名的写调用（包括无分号形式）、可选链、动态成员调用、对象式 `callTool({ name: ... })`，以及操作名无法静态确定的变量化 `callTool` / `invoke` 分派，审计在静态分析能力边界内采取保守报警；明显的类方法与接口签名不视为调用。JS 注释定界符只在字符串之外生效，字符串内容和 URL 中的 `://` 不能改变后续代码的注释状态；普通 JS 字符串、HTML / Markdown 文档文本、JS 块注释、Markdown / HTML 纯注释、Markdown 行内代码与围栏代码示例及惰性字符串示例不按可执行代码处理，注释或示例边界以外的真实调用仍会被扫描。
