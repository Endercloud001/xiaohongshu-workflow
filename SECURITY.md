...
`audit:security` 明确以待提交的 Git index 为扫描对象：用 `git ls-files --cached` 取 index 路径，并用 `git show :path` 读取每个文件的暂存内容，不读工作树副本。无法列出 index 或无法读取任一 index 对象时，审计以失败退出，不会跳过该文件。凭据占位符必须完整匹配整个值，不能以前缀或子串形式夹带真实内容。对写能力的别名调用、无分号调用和变量化 `callTool` / `invoke` 分派，审计在静态分析能力边界内采取保守报警；纯注释、文档禁止语句及惰性字符串示例不按可执行代码处理。
