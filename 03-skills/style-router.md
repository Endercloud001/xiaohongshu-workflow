...
3. 自动路由（默认）
   - 封面：情绪 / 场景 / 人物题 → `cover-anchor-system`（仅在用户本地显式安装后）
   - 封面：纯文字 / 数字冲击 → `cover-anchor-system`（仅在用户本地显式安装后）
   - 封面：中文极密 → `guizang-social-card-skill`（仅在用户本地显式安装后）
   - 内页：强视觉叙事页 → `xhs-visual-director`
   - 内页：清单 / 对比 / 数据 / 密集中文页 → `guizang-social-card-skill`（仅在用户本地显式安装后）
   - 全部失败 → `html-templates`

## 1.1 禁止路线

- 禁止把 `imagegen` / AI 生图得到的无字底图作为背景，再用 HTML/CSS 叠加中文标题、卡片、提示条生成最终图。
- 自动路由时，如选择 AI 生图路线，必须生成“含字成品图”；如选择确定性中文路线，必须选择纯 HTML/guizang 卡片路线，不得混合伪装。guizang 路线仅在本地显式安装后可参与路由。
- Visual Gate 发现文字像后贴图层浮在画面上方时，该页直接判 `revise`。
